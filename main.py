...
def generate_random_exif():
    ...
    # 유명 카메라 제조사와 모델 목록
    camera_models = {
        "Canon": ["Canon EOS 5D Mark IV", "Canon EOS R5", "Canon EOS Rebel T7"],
    ...
    now = datetime.now()
    random_date = now - timedelta(days=random.randint(0, 365*5), hours=random.randint(0,24), minutes=random.randint(0,60))
    ...
    try:
        ...
        for _ in range(int(width * height * 0.01)): # 1%의 픽셀만 변경
            ...
            if img.mode == 'RGB':
                ...
                r, g, b, a = pixels[x, y]
                noise = random.randint(-1, 1)
                pixels[x, y] = (max(0, r + noise), max(0, g + noise), max(0, b + noise), a)
        ...
        if file_ext in ['.jpg', '.jpeg']:
            ...
            for frame in ImageSequence.Iterator(img):
                ...
                for _ in range(int(frame.width * frame.height * 0.01)):
                    x, y = random.randint(0, frame.width - 1), random.randint(0, frame.height - 1)
                    r, g, b = pixels[x, y]
                    noise = random.randint(-1, 1)
                    pixels[x, y] = (max(0, r + noise), max(0, g + noise), max(0, b + noise))
                frames.append(frame)
            frames[0].save(output_path, save_all=True, append_images=frames[1:], duration=img.info.get('duration', 100), loop=0)
        else:
...
class PhotoWasherApp(TkinterDnD.Tk):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(top_frame, text="여기에 파일이나 폴더를 드래그 앤 드롭하세요.", font=("Helvetica", 12)).grid(row=0, column=0, pady=5)
        ...
        btn_wash_all = ttk.Button(bottom_frame, text="전체 세탁", command=self.wash_all_files)
        btn_wash_all.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

        btn_clear = ttk.Button(bottom_frame, text="목록 지우기", command=self.clear_list)
        btn_clear.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

        # 로그 창
        self.log_area = scrolledtext.ScrolledText(self, height=8, wrap=tk.WORD, state='disabled')
    ...
    def _wash_thread_func(self):
        ...
        for i, file_path in enumerate(self.file_list):
            ...
            if os.path.exists(output_path):
                ...
                count = 1
                while os.path.exists(os.path.join(self.output_dir, f"{name} (copy {count}){ext}")):
                    count += 1
                output_path = os.path.join(self.output_dir, f"{name} (copy {count}){ext}")
